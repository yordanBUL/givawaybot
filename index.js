...
client.on('ready', () => {
  ...
  client.user.setActivity(`${prefix}giveaway & ${prefix}help`, { type: 'PLAYING' });
});
...
client.on('message', async message => {
  ...
  message.delete();
  const embed = new Discord.MessageEmbed()
  .setAuthor(client.user.username, client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }), 'https://github.com/notmaxi/giveaway-bot')
  .setThumbnail(client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }))
  .setTitle('Help')
  .setColor('7289da')
  .setDescription(`You want to create a giveaway bot yourself?\n[Check out our Github page!](https://github.com/notmaxi/giveaway-bot)`)
